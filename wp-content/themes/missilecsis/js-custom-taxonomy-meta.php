...
function __js_term_meta_scripts()  {
    ...
    // add theme scripts
    wp_enqueue_script( 'js-custom-taxonomy', get_template_directory_uri() . '/js/js-custom-taxonomy-meta.js', array( 'jquery' ), '', true );
...
add_action( 'missiledb_add_form_fields', '___add_form_field_countries_feature_image' );
...
function ___add_form_field_countries_feature_image() { ?>
    ...
    <div class="form-field countries-feature-image-wrap">
        ...
        <input type="text" name="countries_feature_image" id="countries-feature-image" value="" class="countries-feature-image-field" />
    </div>
<?php }
...
add_action( 'missiledb_edit_form_fields', '___edit_form_field_countries_feature_image' );
...
function ___edit_form_field_countries_feature_image( $term ) {
    ...
    <tr class="form-field countries-feature-image-wrap">
        ...
        <td>
            <?php wp_nonce_field( basename( __FILE__ ), 'countries_feature_image_nonce' ); ?>
            <input type="text" name="countries_feature_image" id="countries-feature-image" value="<?php echo esc_attr( $value ); ?>" class="countries-feature-image-field"  />
            <input type="button" id="meta-image-button" class="button" value="<?php _e( 'Choose or Upload an Image', 'text_domain' )?>" />
        </td>
    </tr>
...
add_action( 'edit_missiledb',   '___save_countries_feature_image' );
add_action( 'create_missiledb', '___save_countries_feature_image' );
...
add_filter( 'manage_edit-missiledb_columns', '___edit_term_columns', 10, 3 );
...
add_filter( 'manage_missiledb_custom_column', '___manage_term_custom_column', 10, 3 );
...
function ___manage_term_custom_column( $out, $column, $term_id ) {
    ...
    if ( 'countries_feature_image' === $column ) {
        ...
        $out = sprintf( '<span class="countries-feature-image-block" style="" >%s</div>', esc_attr( $value ) );
    }
